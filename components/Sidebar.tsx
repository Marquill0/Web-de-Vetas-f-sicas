...
const Sidebar: React.FC<SidebarProps> = ({ currentView, setView, collapsed, setCollapsed, onLogout }) => {
  ...
  return (
    <aside className={`bg-[#0f172a] text-white transition-all duration-300 flex flex-col h-full ${collapsed ? 'w-20' : 'w-64'}`}>
      <div className="p-4 flex items-center justify-between border-b border-slate-700">
        {!collapsed && <span className="text-xl font-bold tracking-tight bg-gradient-to-r from-emerald-400 to-cyan-400 bg-clip-text text-transparent">Gestión Pro</span>}
